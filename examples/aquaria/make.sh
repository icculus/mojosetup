...
if [ ! -f data/_mods/jukebox.xml ]; then
    echo "We don't see data/_mods/jukebox.xml ..."
...
rm -rf image aquaria-installer pdata.zip
mkdir image
mkdir image/guis
mkdir image/scripts
mkdir image/data
...
mv ./mojosetup ../examples/aquaria/aquaria-installer
for feh in *.so *.dll *.dylib ; do
    if [ -f $feh ]; then
        if [ "$DEBUG" != "1" ]; then
            strip $feh
        fi
        mv $feh ../examples/aquaria/image/guis
    fi
done

# Compile the Lua scripts, put them in the base archive.
for feh in ../scripts/*.lua ; do
    ./mojoluac $LUASTRIPOPT -o ../examples/aquaria/image/scripts/${feh}c $feh
done

# Don't want the example config...use our's instead.
rm -f ../examples/aquaria/image/scripts/config.luac
./mojoluac $LUASTRIPOPT -o ../examples/aquaria/image/scripts/config.luac ../examples/aquaria/scripts/config.lua

# Don't want the example app_localization...use our's instead.
rm -f ../examples/aquaria/image/scripts/app_localization.luac
./mojoluac $LUASTRIPOPT -o ../examples/aquaria/image/scripts/app_localization.luac ../examples/aquaria/scripts/app_localization.lua
...
if [ "$OSTYPE" = "Darwin" ]; then
    ...
    mv aquaria-installer "${APPBUNDLE}/Contents/MacOS/mojosetup"
    ...
    ../../cmake-build/make_self_extracting ./aquaria-installer pdata.zip
    rm -f pdata.zip
fi
