...
if [ ! -f data/data/pipes/pipe01i-valve.strip ]; then
    echo "We don't see data/data/pipes/pipe01i-valve.strip ..."
...
rm -rf image cogs-installer pdata.zip
mkdir image
mkdir image/guis
mkdir image/scripts
mkdir image/data
...
mv ./mojosetup ../examples/cogs/cogs-installer
for feh in *.so *.dll *.dylib ; do
    if [ -f $feh ]; then
        if [ "$DEBUG" != "1" ]; then
            strip $feh
        fi
        mv $feh ../examples/cogs/image/guis
    fi
done

# Compile the Lua scripts, put them in the base archive.
for feh in ../scripts/*.lua ; do
    ./mojoluac $LUASTRIPOPT -o ../examples/cogs/image/scripts/${feh}c $feh
done

# Don't want the example config...use our's instead.
rm -f ../examples/cogs/image/scripts/config.luac
./mojoluac $LUASTRIPOPT -o ../examples/cogs/image/scripts/config.luac ../examples/cogs/scripts/config.lua

# Don't want the example app_localization...use our's instead.
rm -f ../examples/cogs/image/scripts/app_localization.luac
./mojoluac $LUASTRIPOPT -o ../examples/cogs/image/scripts/app_localization.luac ../examples/cogs/scripts/app_localization.lua
...
if [ "$OSTYPE" = "Darwin" ]; then
    ...
    mv cogs-installer "${APPBUNDLE}/Contents/MacOS/mojosetup"
    ...
    ../../cmake-build/make_self_extracting ./cogs-installer pdata.zip
    rm -f pdata.zip
fi
