...
if [ ! -f data/Objects/Weapons/blade_hisaian.seria ]; then
    echo "We don't see data/Objects/Weapons/blade_hisaian.seria ..."
...
rm -rf image hammerfight-installer pdata.zip
mkdir image
mkdir image/guis
mkdir image/scripts
mkdir image/data
...
mv ./mojosetup ../examples/hammerfight/hammerfight-installer
for feh in *.so *.dll *.dylib ; do
    if [ -f $feh ]; then
        if [ "$DEBUG" != "1" ]; then
            strip $feh
        fi
        mv $feh ../examples/hammerfight/image/guis
    fi
done

# Compile the Lua scripts, put them in the base archive.
for feh in ../scripts/*.lua ; do
    ./mojoluac $LUASTRIPOPT -o ../examples/hammerfight/image/scripts/${feh}c $feh
done

# Don't want the example config...use our's instead.
rm -f ../examples/hammerfight/image/scripts/config.luac
./mojoluac $LUASTRIPOPT -o ../examples/hammerfight/image/scripts/config.luac ../examples/hammerfight/scripts/config.lua

# Don't want the example app_localization...use our's instead.
rm -f ../examples/hammerfight/image/scripts/app_localization.luac
./mojoluac $LUASTRIPOPT -o ../examples/hammerfight/image/scripts/app_localization.luac ../examples/hammerfight/scripts/app_localization.lua

# Fill in the rest of the Base Archive...
cd ../examples/hammerfight
...
if [ "$OSTYPE" = "Darwin" ]; then
    ...
    mv hammerfight-installer "${APPBUNDLE}/Contents/MacOS/mojosetup"
    ...
    ../../cmake-build/make_self_extracting ./hammerfight-installer pdata.zip
    rm -f pdata.zip
fi
