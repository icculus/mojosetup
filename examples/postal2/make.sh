...
if [ ! -f data/System/postal2-bin ]; then
    echo "We don't see data/System/postal2-bin ..."
...
APPNAME="Postal 2: Share the Pain Installer"
...
rm -rf image postal2-installer pdata.zip
mkdir image
mkdir image/guis
mkdir image/scripts
mkdir image/data
...
mv ./mojosetup ../examples/postal2/postal2-installer
for feh in *.so *.dll *.dylib ; do
    if [ -f $feh ]; then
        if [ "$DEBUG" != "1" ]; then
            strip $feh
        fi
        mv $feh ../examples/postal2/image/guis
    fi
done

# Compile the Lua scripts, put them in the base archive.
for feh in ../scripts/*.lua ; do
    ./mojoluac $LUASTRIPOPT -o ../examples/postal2/image/scripts/${feh}c $feh
done

# Don't want the example config...use our's instead.
rm -f ../examples/postal2/image/scripts/config.luac
./mojoluac $LUASTRIPOPT -o ../examples/postal2/image/scripts/config.luac ../examples/postal2/scripts/config.lua

# Don't want the example app_localization...use our's instead.
rm -f ../examples/postal2/image/scripts/app_localization.luac
./mojoluac $LUASTRIPOPT -o ../examples/postal2/image/scripts/app_localization.luac ../examples/postal2/scripts/app_localization.lua
...
if [ "$OSTYPE" = "Darwin" ]; then
    ...
    mv postal2-installer "${APPBUNDLE}/Contents/MacOS/mojosetup"
    ...
    ../../cmake-build/make_self_extracting ./postal2-installer pdata.zip
    rm -f pdata.zip
fi
