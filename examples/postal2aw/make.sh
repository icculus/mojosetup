...
if [ ! -f data/ApocalypseWeekend/System/AW7FX.u ]; then
    echo "We don't see data/ApocalypseWeekend/System/AW7FX.u ..."
...
APPNAME="Postal 2: Apocalypse Weekend Installer"
...
rm -rf image postal2aw-installer pdata.zip
mkdir image
mkdir image/guis
mkdir image/scripts
mkdir image/data
...
mv ./mojosetup ../examples/postal2aw/postal2aw-installer
for feh in *.so *.dll *.dylib ; do
    if [ -f $feh ]; then
        if [ "$DEBUG" != "1" ]; then
            strip $feh
        fi
        mv $feh ../examples/postal2aw/image/guis
    fi
done

# Compile the Lua scripts, put them in the base archive.
for feh in ../scripts/*.lua ; do
    ./mojoluac $LUASTRIPOPT -o ../examples/postal2aw/image/scripts/${feh}c $feh
done

# Don't want the example config...use our's instead.
rm -f ../examples/postal2aw/image/scripts/config.luac
./mojoluac $LUASTRIPOPT -o ../examples/postal2aw/image/scripts/config.luac ../examples/postal2aw/scripts/config.lua

# Don't want the example app_localization...use our's instead.
rm -f ../examples/postal2aw/image/scripts/app_localization.luac
./mojoluac $LUASTRIPOPT -o ../examples/postal2aw/image/scripts/app_localization.luac ../examples/postal2aw/scripts/app_localization.lua

# Fill in the rest of the Base Archive...
cd ../examples/postal2aw
...
if [ "$OSTYPE" = "Darwin" ]; then
    ...
    mv postal2aw-installer "${APPBUNDLE}/Contents/MacOS/mojosetup"
    ...
    ../../cmake-build/make_self_extracting ./postal2aw-installer pdata.zip
    rm -f pdata.zip
fi
