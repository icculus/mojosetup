...
if [ ! -f data/Fonts/Chinese.fnt ]; then
    echo "We don't see data/Fonts/Chinese.fnt ..."
...
APPNAME="Super Meat Boy Installer"
...
rm -rf image supermeatboy-installer pdata.zip
mkdir image
mkdir image/guis
mkdir image/scripts
mkdir image/data
...
mv ./mojosetup ../examples/supermeatboy/supermeatboy-installer
for feh in *.so *.dll *.dylib ; do
    if [ -f $feh ]; then
        if [ "$DEBUG" != "1" ]; then
            strip $feh
        fi
        mv $feh ../examples/supermeatboy/image/guis
    fi
done

# Compile the Lua scripts, put them in the base archive.
for feh in ../scripts/*.lua ; do
    ./mojoluac $LUASTRIPOPT -o ../examples/supermeatboy/image/scripts/${feh}c $feh
done

# Don't want the example config...use our's instead.
rm -f ../examples/supermeatboy/image/scripts/config.luac
./mojoluac $LUASTRIPOPT -o ../examples/supermeatboy/image/scripts/config.luac ../examples/supermeatboy/scripts/config.lua

# Don't want the example app_localization...use our's instead.
#rm -f ../examples/supermeatboy/image/scripts/app_localization.luac
#./mojoluac $LUASTRIPOPT -o ../examples/supermeatboy/image/scripts/app_localization.luac ../examples/supermeatboy/scripts/app_localization.lua

# Fill in the rest of the Base Archive...
cd ../examples/supermeatboy
...
if [ "$OSTYPE" = "Darwin" ]; then
    ...
    mv supermeatboy-installer "${APPBUNDLE}/Contents/MacOS/mojosetup"
    ...
    ../../cmake-build/make_self_extracting ./supermeatboy-installer pdata.zip
    rm -f pdata.zip
fi
